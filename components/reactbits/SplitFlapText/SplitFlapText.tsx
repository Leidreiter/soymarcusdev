...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const updateTiles = (updates: TileUpdate[]) => {
        setTiles((previous) => {
          ...
          while (nextTiles.length < targetLength) {
            nextTiles.push({ current: " ", next: " ", flipping: false, tick: 0 });
          }
          ...
          if (step < plan.sequence.length) {
            ...
            if (step !== plan.step) {
              ...
              updates.push({
                index: plan.index,
                current:
                  step === 0 ? plan.from : plan.sequence[step - 1],
    ...
    const startCycling = (delay: number) => {
    ...
    if (isFirstRun) {
      const scramble = Array.from(firstPhrase)
        .map((char) => (char === " " ? " " : sampleChar(activeCharset)))
        .join("");
      currentTextRef.current = scramble;
      setTiles(createTiles(scramble));
      if (scramble === firstPhrase) {
        return clearAnimation;
      }
      cycleTimerRef.current = window.setTimeout(() => {
        const duration = animateTo(firstPhrase);
        startCycling(scrambleHoldMs + duration + safeCycleDelay);
      }, scrambleHoldMs);
    } else {
      const duration = animateTo(firstPhrase);
      startCycling(duration + safeCycleDelay);
    }
