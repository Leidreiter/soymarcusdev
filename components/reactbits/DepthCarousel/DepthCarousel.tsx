...
export type DepthCarouselItem =
  | string
  | { image: string; alt?: string; href?: string };
...
export interface DepthCarouselProps {
  ...
  onChange?: (index: number, item: { image: string; alt?: string; href?: string }) => void;
  className?: string;
}
...
const normalizeItem = (it: DepthCarouselItem) =>
  typeof it === 'string' ? { image: it, alt: '', href: undefined } : it;
...
const DepthCarousel = ({
  ...
  const onLinkClick = useCallback(
    (index: number) => (e: ReactMouseEvent<HTMLAnchorElement>) => {
      const suppress = suppressRef.current;
      suppressRef.current = false;
      if (index !== active || suppress) {
        e.preventDefault();
        e.stopPropagation();
      }
    },
